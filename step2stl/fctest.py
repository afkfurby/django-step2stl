...
def stp_converter(filein, fileout=""):

    fileout = filein.replace(".step", "").replace(".stp", "").replace(".STEP", "").replace(".STP", "") + ".stl"

    with open(os.path.join(settings.BASE_DIR, 'step2stl/freecad/freecad_step2stl.py')) as Script:
...
output = stp_converter(os.path.join(settings.BASE_DIR, 'media/test.step'))
